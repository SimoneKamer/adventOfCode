...
public class ToFirstIntersectionCalculator extends AbstractCalculator {
    Integer findDistanceToHq(String sequenceOfInstructions) {

        List<String> instructions = Arrays.asList(sequenceOfInstructions.split(", "));
        int index = 0;
        while (locationOfHQ == null) {
            followInstruction(instructions.get(index));
            index++;
        }
        return calculateDistance(locationOfHQ);
    }

    public static void main(String[] args) {
        ToFirstIntersectionCalculator calculator = new ToFirstIntersectionCalculator();
